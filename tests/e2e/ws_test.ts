...
Deno.test('E2E: WebSocket - Connection & Subscriptions', async () => {
  ...
  const tickerPromise = new Promise<TickerWsMessage>((resolve) => {
    client.ws.subscribe({ type: 'ticker', symbol: 'BTC-USD' }, (msg) => {
      if (msg.topic === 'ticker.BTC-USD') resolve(msg as TickerWsMessage)
    })
  })
  ...
  const candlePromise = new Promise<CandleWsMessage>((resolve) => {
    client.ws.subscribe({ type: 'candle', symbol: 'BTC-USD', interval: '1m' }, (msg) => {
      if (msg.topic === 'candle.BTC-USD.1m') resolve(msg as CandleWsMessage)
    })
  })
