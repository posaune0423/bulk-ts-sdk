...
Deno.test('E2E: Trade - Limit Order Flow', async () => {
  ...
  const openOrders = await client.account.openOrders(client.accountId!)
  const found = openOrders.find((o) => o.oid === orderId)
  ...
  const openOrdersAfter = await client.account.openOrders(client.accountId!)
  const foundAfter = openOrdersAfter.find((o) => o.oid === orderId)
...
Deno.test('E2E: Trade - Cancel All', async () => {
  ...
  const openOrders = await client.account.openOrders(client.accountId!)
  const btcOrders = openOrders.filter((o) => o.coin === 'BTC-USD')
